...
const OrderPage = () => {
  ...
  const handleSellOrders = async () => {
    ...
    const ordersToSell = selectedOrders.map((orderId) => ({
      orderId,
      quantitySold: quantityToSell[orderId] || 0,
    }));
  
    try {
      // Send sell request to the backend
      const response = await fetch('http://localhost:5000/api/orders/sell', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ ordersToSell }),
      });
  
      const data = await response.json();
      if (response.ok) {
        // If the sell operation was successful, update frontend state
        ...
        setErrorMessage(data.error || 'Error selling orders.');
      }
    } catch (error) {
      console.error('Error selling orders:', error);
      setErrorMessage('Error selling orders.');
    }
  };
              ...
              {orders.map((order) => (
                <div key={order._id} className="order-item">
                  ...
                  <div className="order-details">
                    ...
                    <p>
                      Price: <span className="order-price">₹{order.price.toFixed(2)}</span>
                    </p>
                    <p>
                      Quantity: <span className="order-quantity">{order.quantity}</span>
                    </p>
                    <p>
                      Total: <span className="order-total">₹{order.total.toFixed(2)}</span>
                    </p>
